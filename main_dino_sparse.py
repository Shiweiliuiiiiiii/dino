...
def get_args_parser():
    ...
    parser.add_argument('--kernel_size', nargs="*", type=int, default=[51, 49, 47, 13, 5],
                        help='kernel size of conv [stage1, stage2, stage3, stage4, N]')
    ...
    parser.add_argument('--growth', type=str, default='random',
                        help='Growth mode. Choose from: momentum, random, gradient.')
    ...
    parser.add_argument('--fix', action='store_true',
                        help='Fix sparse model during training i.e., no weight adaptation.')
    parser.add_argument('--sparse_init', type=str, default='snip', help='layer-wise sparsity ratio')
    parser.add_argument('-u', '--update-frequency', type=int, default=100, metavar='N',
                        help='how many iterations to adapt weights')
...
def train_dino(args):
    ...
    mask = None
    if args.sparse:
        decay = CosineDecay(args.prune_rate, len(data_loader) * args.epochs,
                            init_step=len(data_loader) * start_epoch)

        mask = Masking(optimizer, train_loader=data_loader, prune_mode=args.prune, prune_rate_decay=decay,
                       growth_mode=args.growth, redistribution_mode=args.redistribution, fp16=args.use_fp16, args=args)
        mask.add_module(student)
