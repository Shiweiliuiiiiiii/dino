...
def train_dino(args):
    ...
    mask=None
    if args.sparse:
        decay = CosineDecay(args.prune_rate, len(data_loader)*args.epochs, init_step= len(data_loader)*start_epoch)
        mask = Masking(optimizer, train_loader=data_loader, prune_mode=args.prune, prune_rate_decay=decay, growth_mode=args.growth, redistribution_mode=args.redistribution, args=args)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(student, teacher, teacher_without_ddp, dino_loss,
            ...
            epoch, fp16_scaler, args, mask=mask)
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, data_loader,
                    ...
                    fp16_scaler, args, mask):
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Epoch: [{}/{}]'.format(epoch, args.epochs)
    for it, (images, _) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        if it==0:
            snip_sparsity = SNIP(student, teacher, fp16_scaler, 1-mask.sparsity, images, mask, dino_loss, args)
